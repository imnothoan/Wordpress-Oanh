 ...
 */
if (!function_exists('interior_design_firm_is_plugin_installed')) {
    function interior_design_firm_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('interior_design_firm_is_plugin_activated')) {
    function interior_design_firm_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_interior_design_firm_install_and_activate_plugins', 'interior_design_firm_install_and_activate_plugins');
add_action('wp_ajax_nopriv_interior_design_firm_install_and_activate_plugins', 'interior_design_firm_install_and_activate_plugins');
add_action('wp_ajax_interior_design_firm_rplugin_activation', 'interior_design_firm_rplugin_activation');
add_action('wp_ajax_nopriv_interior_design_firm_rplugin_activation', 'interior_design_firm_rplugin_activation');
...
function interior_design_firm_all_plugins_active() {
...
function interior_design_firm_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('interior_design_firm_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_interior_design_firm_plugin_installed($plugin_slug)) {
...
function is_interior_design_firm_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_interior_design_firm_dismissed_notice_handler', 'interior_design_firm_ajax_notice_dismiss_function');

function interior_design_firm_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'interior_design_firm_welcome_nonce')) {
...
function interior_design_firm_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        $interior_design_firm_current_screen = get_current_screen();
        $interior_design_firm_theme = wp_get_theme();
        if ($interior_design_firm_current_screen && $interior_design_firm_current_screen->id !== 'appearance_page_interior-design-firm-guide-page') {
            ?>

            <div class="interior-design-firm-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="interior-design-firm-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($interior_design_firm_theme->get('Name')); ?></p>
                        <p><?php _e('For information and detailed instructions, check out our theme documentation.', 'interior-design-firm'); ?></p>
                    </div>
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'interior-design-firm'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
...
add_action('admin_notices', 'interior_design_firm_custom_admin_notice');

// After switching theme, reset dismissed notice option
add_action('after_switch_theme', 'interior_design_firm_after_switch_theme');
function interior_design_firm_after_switch_theme() {
