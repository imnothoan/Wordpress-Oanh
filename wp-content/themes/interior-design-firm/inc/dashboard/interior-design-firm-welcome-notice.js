(function ($) {
    ...
    $("#install-activate-button").on("click", function (e) {
        ...
        var activationData = {
            action: "interior_design_firm_install_and_activate_plugins",
            nonce: interior_design_firm_localize.nonce,
        };

        $.post(interior_design_firm_localize.ajax_url, activationData, function (response) {
            console.log("asdasd", response);
            if (response.success) {
                window.location.href = interior_design_firm_localize.redirect_url;
            } else {
    ...
    $(document).on('click', '.notice-info .notice-dismiss', function () {
        ...
        $.ajax({
            type: 'POST',
            url: interior_design_firm_localize.ajax_url,
            data: {
                action: 'interior_design_firm_dismissed_notice_handler',
                type: type,
                wpnonce: interior_design_firm_localize.dismiss_nonce
            },
